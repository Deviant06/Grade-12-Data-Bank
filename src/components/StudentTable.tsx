...
const StudentTable: React.FC<StudentTableProps> = ({ students }) => {
  ...
  const gradeOptions = Array.from(new Set(students.map(s => s.grade))).sort();
  ...
  const handleAction = (action: string, student: Student) => {
    switch (action) {
      case 'edit':
        toast({
          title: "Edit Student",
          description: `Editing ${student.name}'s record`,
        });
        break;
      case 'delete':
        toast({
          title: "Delete Student",
          description: `${student.name}'s record would be deleted`,
          variant: "destructive",
        });
        break;
      default:
